...
export default function ClientPortalDashboard() {
  ...
  async function fetchClientData() {
    ...
    try {
      ...
      // ✅ FIXED: merge family_members and documents into client object
      setClient({
        ...data.client,
      ...
      <nav className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-50 shadow-sm">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-bold text-xs hover:bg-slate-50 transition-colors flex items-center gap-2"
          >
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            {client.family_members && client.family_members.length > 0 && (
              <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  {client.family_members.map((member: any) => (
                    <div key={member.id} className="flex items-center gap-4 p-3 rounded-2xl bg-slate-50 border border-slate-100">
                      <div className="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center font-bold text-slate-400 text-lg">
                        {member.name[0]}
                      </div>
                      <div>
